print("Hello Bwee Bwee")
print(5+5)
print("5+5 equals 10")
print(100/5) #end of useless part
x, y, a, b = 100-10, 1050-102, 2035817-1203, 21230358-223
print(x)
print(y)
print(y-x)
if x > 5:
    print("X is more than 5!")
if y < x:
    print("Y is less than X!") #dunno why i even have this in here cause it doesn't do anything
print(a)#checking to see if code was working
print(b)
print(a+b)
print(x+y)#end of checking
if a+b > x+y:
    print("A+B is more than x+y!")
H, h = "johnny", "appleseed"
print(H)
print(h)
print(H+h)
#Guys i am like a god at coding!!
BweeBweeIsTheBest, FatAssCat, Sarga = "Fish breath attack", " Candy", " MEOOOOOOOOOWW"
print(BweeBweeIsTheBest)
print(FatAssCat)
print(Sarga)
print(BweeBweeIsTheBest+FatAssCat+Sarga)
#end of making fun of my cats
#
#
#
def myfunc():
  global x
  x = 'bwee bwee '
myfunc()
print('Python is ' + x)
print(x)
print(h,x)
def bweebwee():
    global y
    y = "is the best"
    print(y)
bweebwee()
print(x+y)
print(y)